...
describe Snowly::SchemaCache do
  ...
  context 'with external schema' do
    ...
    let(:file_content)  { File.read(File.expand_path('../../fixtures/snowplow_context.json', __FILE__)) }
  ...
  context 'with local schema' do
    let(:location) { 'iglu:test_context.json'}
    let(:file_content) { File.read(File.expand_path('../../fixtures/test_context.json', __FILE__)) }
    context 'when already loaded' do
